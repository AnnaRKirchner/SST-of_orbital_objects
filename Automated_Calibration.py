...
Description:
    ...
    More Information in ReadMe or Thesis paper
...
def plot_distortion(img, pointsA, pointsB, wcs):
    ...
    plt.scatter(distanceB, offset, c="red", s=100, label="distorted image points")
    plt.plot(r_line, y_line, '--', color ='blue', label ="nonlinear regression with approximation function: \n $f(x)=k_1\cdot x+k_2\cdot x^3+k_3\cdot x^5$ ")
    plt.errorbar(distanceB, offset, xerr=u, yerr=u, fmt='.k',  capsize=6, color= 'red')
    #plt.title(label="Distortion Function")
    plt.xlabel('Distance to reference projection point (px)', fontsize=50, labelpad=10)
    plt.ylabel('Distortion (px)', fontsize=50, labelpad=15)
    plt.legend(fontsize=40)
    plt.yticks(fontsize=40)
    plt.xticks(fontsize=40)
    plt.show()
    plt.savefig('C:\\Users\\annar\\Informatik\\Standard_Frame\\Processing\\RadialDist.jpg')
...
textfile= r'C:\Users\annar\Informatik\Standard_Frame\Rest\Andoya2.txt'
wcsfile= r'C:\Users\annar\Informatik\Standard_Frame\Rest\wcs.fits'
